<?php
   ...
   require_once '../../../../configs/2023/veg/aaatest/quickbooks.php';
   ...
   if($_GET["action"] === 'synchVegInvoice'){
      ...
      foreach($invoiceHeaderResults as $invoiceHeaderRow){
         ...
         $invoicequeue = new QuickBooks_WebConnector_Queue('mysqli://IT_ADMIN:sysadmin2018@192.168.1.170:3306/testvegaaa2023');
         ...
         foreach($invoiceLineResults as $invoiceLineRow){
            ...
            $unitrate = $invoiceLineRow[3]>0 ? $invoiceLineRow[5]/$invoiceLineRow[3] : 0;
            ...
            foreach($productResults as $productRow){
               ...
               $subitem=str_replace(" ","",substr($productRow[2], 0, 29))."x".$productRow[7];
               if($productRow[7]<1){
            ...
            // $itemfullname='Flamingo Produce Ltd'.":".$flamingoitems.":"."".$rowd[1];
         
            $itemfullname=$flamingoitems.":".$subitem;

            $other1 = ($netweightkg * $invoiceLineRow[2] * $invoiceLineRow[3]).'Kgs net';
