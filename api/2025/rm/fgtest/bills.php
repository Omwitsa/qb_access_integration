<?php
   ...
   if($_GET["action"] === 'syncRmBills'){
      ...
      foreach($billsResults as $billRow){
         ...
         $agentQuery = "SELECT HandlingAgentId, QBHandlingAgentNameFG FROM HandlingAgent WHERE HandlingAgentId = $clearingAgentId";
         ...
         if(!empty($invoiceNo)){
            ...
            $billqueue = new QuickBooks_WebConnector_Queue('mysqli://IT_ADMIN:sysadmin2018@192.168.1.170:3306/testrosesfg');
   ...
   if($_GET["action"] === 'getRMFgBillsStats'){
      ...
      $stagedCountQuery = "SELECT COUNT(*) FROM qb_bill WHERE TimeModified IS NULL;";
