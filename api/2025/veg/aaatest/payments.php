<?php
   ...
   if($_GET["action"] === 'syncVegPayments'){
      ...
      foreach($custPaymentResults as $custPaymentRow){
         ...
         if(!empty($qbCustName)){
            ...
            $paymentqueue = new QuickBooks_WebConnector_Queue('mysqli://IT_ADMIN:sysadmin2018@192.168.1.170:3306/testvegaaa2025');
   ...
   if($_GET["action"] === 'getVegaaaPaymentsStats'){
      $results["items"] = array();
      ...
      $unsynchedCountQuery = "SELECT COUNT(*) FROM CustomerPayment WHERE ForeignAmountPaid > 0 AND ExporterId = 1 AND QBTransferStatus IS NULL AND PaymentDate Between #5/19/2025# AND #12/31/2026#";
