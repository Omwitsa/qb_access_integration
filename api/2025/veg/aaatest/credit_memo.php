<?php
   ...
   if($_GET["action"] === 'syncVegCreditNotes'){
      ...
      foreach($creditNoteResults as $creditNoteRow){
         ...
         $qbIdQuery = "SELECT TxnID FROM qb_creditmemo WHERE TxnID = '$txnID';";
         ...
         if(!empty($qbCustName)){
            ...
            foreach($claimLineResults as $claimLineRow){
               ...
               $itemfullname=$flamingoitems.":".$subitem; // Veggetables, Flowers, Roses
               array_push($creditNoteLines, "('$txnID', '80DA4-1749202608', '$itemfullname', '$descrip', $quantity, $rate, $lineAmount, '$taxName')");
            }

            $strCreditNoteLines = implode(',', $creditNoteLines);
            if($strCreditNoteLines){
               $insertCreditNoteQuery = "INSERT INTO qb_creditmemo_creditmemoline(CreditMemo_TxnID, TxnLineID, Item_FullName, Descrip, Quantity, Rate, Amount, SalesTaxCode_FullName) VALUES $strCreditNoteLines;";
