<?php
   ...
   if($_GET["action"] === 'synchVegInvoice'){
      ...
      $invoiceHeaderQuery = "SELECT InvoiceHeaderId, CustomerId, InvoiceDate, InvoiceNo, ShippingTerms, FlightDate, QBInvoiceNo, Ref FROM InvoiceHeader WHERE Finalized = Yes AND ExporterId = 1 AND InvoiceDate Between #1/1/2026# AND #31/12/2026# ORDER BY InvoiceHeaderId";
   ...
   if($_GET["action"] === 'getVegAAAInvoicesStats'){
      ...
      $unsynchedInvoiceCountQuery = "SELECT COUNT(*) FROM InvoiceHeader WHERE Finalized = Yes AND ExporterId = 1 AND  QBTransferStatus IS NULL AND InvoiceDate Between #1/1/2026# AND #31/12/2026#";
